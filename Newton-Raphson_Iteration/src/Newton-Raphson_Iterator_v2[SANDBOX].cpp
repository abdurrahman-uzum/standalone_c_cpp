...
*/
void scan( int x0, int y0, int w, int h )
{
	for( int y = ylim; y > -ylim; y-- )
	{
		for( int x = -xlim; x < xlim; x++ )
		{
			...
			switch ( iterate( complex(x/ppul, y/ppul) ) )
			{
				...
				default:
					SDL_SetRenderDrawColor( renderer, 0x00, 0x00, 0x00, 0xff ); //Black: iteration diverged.
			}
...
void scan2( double x0, double y0, double d )
{
	double res = SCREEN_WIDTH / d;

	for ( double y = y0 + d;  y > y0 - d;  y -= res )
	{
		for( double x = x0 - d;  x < x0 + d;  x += res)
		{
			switch ( iterate( complex(x, y) ) )
			{
				case 1:
					SDL_SetRenderDrawColor( renderer, 0xff, 0x00, 0x00, 0xff ); //Red
				break;
			...
			SDL_RenderDrawPoint( renderer, X(x*res), Y(y*res) );
...
int main ( int argc, char* argv[] )
{
	...
	complex scan_origin = complex( 0, 0 );
	...
	SDL_RenderDrawCircle( renderer, SCREEN_WIDTH/2, SCREEN_HEIGHT/2, 0.8*ppul );
	SDL_RenderDrawCircle( renderer, SCREEN_WIDTH/2, SCREEN_HEIGHT/2, 1.44*ppul );
	SDL_RenderDrawCircle( renderer, SCREEN_WIDTH/2, SCREEN_HEIGHT/2, 2.255*ppul );
	SDL_RenderDrawCircle( renderer, SCREEN_WIDTH/2, SCREEN_HEIGHT/2, 3.42*ppul );
	SDL_RenderDrawCircle( renderer, SCREEN_WIDTH/2, SCREEN_HEIGHT/2, 5.15*ppul );
	SDL_RenderDrawCircle( renderer, SCREEN_WIDTH/2, SCREEN_HEIGHT/2, 7.72*ppul );
