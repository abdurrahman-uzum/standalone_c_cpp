...
void printMaze()
{
    ...
    for (int i=0; i<SIZE; i++)
    {
        ...
        {
            if ( maze[i][j] == WALL || maze[i][j] == BLOCK || maze[i][j] == CHARACTER )
            {
                printf(" %c ", maze[i][j]);
                continue;
            }
            ...
            {
                maze[i][j] > 99 ? printf("%d", maze[i][j]) : printf(" %d", maze[i][j]);
            }
            else
            {
                printf(" %d ", maze[i][j]);
...
int followTrace( int currentDirection )
{
    ...
    int up = (int)maze[y-1][x] != 254 ? (int)maze[y-1][x] :0;

    int down = (int)maze[y+1][x] != 254 ? (int)maze[y+1][x] :0;

    int right = (int)maze[y][x+1] != 254 ? (int)maze[y][x+1] :0;

    int left = (int)maze[y][x-1] != 254 ? (int)maze[y][x-1] :0;
...
void penmark ( int IncrDecr )
{
    if ( IncrDecr == '+' )
    {
        for ( int i=0; i<SIZE; i++ )
        {
            for ( int j=0; j<SIZE; j++ )
            {
                if ( maze[i][j]!=0 && maze[i][j]!=WALL && maze[i][j]!=BLOCK && maze[i][j]!=CHARACTER )
                {
                    maze[i][j] = (int)maze[i][j] + 5;
                }
            }
        }
    }
...
int main()
{
    ...
    while ( !done )
    {
        ...
        while ( decision > 0 && !done )
        {
            ...
            if ( whatsThere(direction) > 0 && whatsThere(direction)!=254 && whatsThere(direction)!=BLOCK && numberOfChoices() == 2 )
            {
                numberOfLoops++;
                printf("\n Loop detected.  ");
                system("pause");
