...
bool loadMedia()
{
    ...
    keySurfaces[DEFAULT] = loadSurface("SDL_Tutorial_Media/keyPressesMedia/wtfSmall.bmp");
            ...
            while ( !quit )
            {
                while ( SDL_PollEvent(&event) != 0 )
                {
                    ...
                    else if ( event.type == SDL_KEYDOWN )
                    {
                        ...
                        default:
                            currentSurface = keySurfaces[DEFAULT];
                            printf("HOW DARE YOU PRESS %c !?\n", event.key.keysym.sym);
                            break;
                        }
                    }
                }
                SDL_FillRect ( currentSurface, NULL, SDL_MapRGB ( currentSurface->format, 255,255,255  ));
