...
void turn ( int where )
{
    ...
    else if ( where == BACK )
    {
        ...
        if ( turns[numberOfTurns] == 'R' )
        {
            turns[++numberOfTurns] = 'L';
            turns[++numberOfTurns] = 'L';
        }
        else if ( turns[numberOfTurns] == 'L' )
        {
            turns[++numberOfTurns] = 'R';
            turns[++numberOfTurns] = 'R';
        }
...
bool isloop()
{
    ...
    else if ( abs(loopDetector) > 4 )
    {
        printf("\nMessage from function isloop() : \"Strange things are happening. I recommend fixing the damn code before going further, Sir!\"");
        system("pause");
    }
    return false;
}
...
/// Solve how to modify turns array when it turns back.
